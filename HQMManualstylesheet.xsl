...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="Issue">
		<div class="Issue" background="{img/@src}';" >
			...
			<img width="800" src="C:\Users\Jules Mason\Documents\Work\BREEAM\schema\{img/@src}"/>
	...
	<h2>Credit Summary</h2>
			<table class="Summary" width="800" bgcolor="#003552">
				...
				<tr>
					<td align="left"><xsl:for-each select="Criterion"><xsl:apply-templates select="@id" mode="CriterionTitle"/>,</xsl:for-each></td>
					<td><div><xsl:value-of select="@id"/> - <xsl:value-of select="name"/></div></td>
					<td align="right"><div><xsl:value-of select="sum(Credits/credit)"/></div></td>
				</tr>
				</xsl:for-each>
				<tr>
					<td colspan="2"><b>Total credits availalble</b></td>
					<td align="right"><b><xsl:value-of select="sum(Criteria//Credits/credit)"/></b></td>
				</tr>
			</table>
	</xsl:template>
	<!--Title Object -->
	...
	<xsl:template match="@id" mode="SectionTitle">
		<span>
			<img width="40" src="C:\Users\Jules Mason\Documents\Work\BREEAM\schema\{../icon/@src}"/>  |  <xsl:value-of select="."/> | <xsl:apply-templates select="../name"/>
		</span>
	</xsl:template>
	...
	<xsl:template match="Credits" mode="Credits">
		<div class="Credits">
			<h2 class="Credits" >Max Credits</h2>
				<div class="Credit"><xsl:value-of select="sum(../Criteria/Credits/credit)" /></div>
			<h2 class="Credits" >Indicators (Average)</h2>
			<table>
				<tr height="200px" valign="bottom">
						<xsl:for-each select="Indicators/Indicator/@name">
					<td><img width="100px" height="{sum(../@weight) div count(../@weight) * 200}" src="C:\Users\Jules Mason\Documents\Work\BREEAM\schema\icon\bar.png" /></td>
						</xsl:for-each>
				</tr>
			     <tr>
						<xsl:for-each select="//part/Indicator">
					<td>							<img width="80" src="C:\Users\Jules Mason\Documents\Work\BREEAM\schema\{icon/@src}" />					</td>
						</xsl:for-each>
				</tr>
			
				<tr>
						<xsl:for-each select="Indicators/Indicator/@name">
					<td style="color:#FFFFFF">							<xsl:value-of select="."/>					</td>
						</xsl:for-each>
				</tr>
	...
	<h2>Criteria</h2>
	<xsl:for-each select="Criteria">
	<h3><xsl:value-of select="@id"/> - <xsl:value-of select="name"/></h3>
	<xsl:for-each select="Criterion">
	<p><xsl:apply-templates select="@id" mode="CriterionTitle" /></p>
	...
	<!-- test for processing instrucitons -->
	<xsl:template match="processing-instruction('for-each')">
		<b>
			<xsl:value-of select="."/>
		</b>
	</xsl:template>
	<!-- Draft Project paramters by issue
	<xsl:template match="inputs">
		<h2>
			<img src="icon/home_quality_mark_logo3.png" height="40px"/>
